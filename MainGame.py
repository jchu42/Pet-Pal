...
gm.setState("room") # set first state (should be login screen)
...
#def handleStateChange (self, state, *args, **kwargs): # arbitrary argument input for game state customization
    # if (state == 0): # turn into classes - each state gets its own class, and one class for each object (Button class with function passed in on init, or Meal class with image name passed in on init?)
    ...
    #     gm.assignDelete(mainPet, lambda self:print("*dies*"))
